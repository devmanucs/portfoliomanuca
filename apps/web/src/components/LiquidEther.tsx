...
`;
    ...
    class Advection extends ShaderPass {
      ...
      update({ dt, isBounce, BFECC }: AdvectionUpdate) {
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: ExternalForceUpdate) {
        const forceX = (Mouse.diff.x / 2) * props.mouse_force;
        const forceY = (Mouse.diff.y / 2) * props.mouse_force;
        const cellScale = props.cellScale;
        const cursorSize = props.cursor_size;
    ...
    class Viscous extends ShaderPass {
      ...
      update({ viscous, iterations, dt }: ViscousUpdate): RenderTarget {
        if (!this.uniforms) {
          throw new Error("LiquidEther viscosity uniforms are unavailable.");
        }
        const output0 = this.props.output0;
        const output1 = this.props.output1;
        if (!output0 || !output1) {
          throw new Error("LiquidEther viscosity targets are unavailable.");
        }
        let fboIn = output0;
        let fboOut = output1;
    ...
    class Poisson extends ShaderPass {
      ...
      update({ iterations }: { iterations: number }): RenderTarget {
        const output0 = this.props.output0;
        const output1 = this.props.output1;
        if (!output0 || !output1) {
          throw new Error("LiquidEther pressure targets are unavailable.");
        }
        let pressureIn = output0;
        let pressureOut = output1;
        for (let i = 0; i < iterations; i++) {
          if (i % 2 === 0) {
            pressureIn = output0;
            pressureOut = output1;
    ...
    class Simulation {
      ...
      getTarget(key: string): RenderTarget {
        const target = this.fbos[key];
        if (!target) {
          throw new Error(`LiquidEther render target "${key}" is unavailable.`);
        }
        return target;
      }
      createShaderPass() {
        this.advection = new Advection({
          ...
          dt: this.options.dt,
          src: this.getTarget("vel_0"),
          dst: this.getTarget("vel_1")
        });
        ...
        this.viscous = new Viscous({
          ...
          src: this.getTarget("vel_1"),
          dst: this.getTarget("vel_viscous1"),
          dst_: this.getTarget("vel_viscous0"),
          dt: this.options.dt
        });
        this.divergence = new Divergence({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src: this.getTarget("vel_viscous0"),
          dst: this.getTarget("div"),
          dt: this.options.dt
        });
        this.poisson = new Poisson({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src: this.getTarget("div"),
          dst: this.getTarget("pressure_1"),
          dst_: this.getTarget("pressure_0")
        });
        this.pressure = new Pressure({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src_p: this.getTarget("pressure_0"),
          src_v: this.getTarget("vel_viscous0"),
          dst: this.getTarget("vel_0"),
          dt: this.options.dt
        });
      }
      calcSize() {
      ...
      update() {
        ...
        const initialVelocity = this.fbos.vel_1;
        if (!initialVelocity) return;
        let vel: RenderTarget = initialVelocity;
  ...
  return (
    <div
      ref={mountRef}
      className={`liquid-ether-container pointer-events-none ${className}`}
      style={style}
    />
  );
}
