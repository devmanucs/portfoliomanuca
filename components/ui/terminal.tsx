...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "z-0 h-full max-h-100 w-full max-w-lg overflow-hidden rounded-2xl border",
        glass
          ? "relative border-white/10 bg-background/45 shadow-2xl shadow-sage/10 ring-1 ring-white/10 backdrop-blur-2xl"
          : "border-border bg-background rounded-xl",
        className
      )}
    >
      {glass ? (
        <div className="pointer-events-none absolute inset-0 bg-linear-to-br from-white/8 via-transparent to-sage/8" />
      ) : null}
      ...
      <div
        className={cn(
          ...
          glass
            ? "border-white/10 bg-background/25 backdrop-blur-md"
            : "border-border gap-y-2 p-4"
        )}
      >
        <div className="flex shrink-0 flex-row gap-x-2">
          <div className="h-2.5 w-2.5 rounded-full bg-red-500/90 shadow-[0_0_6px_rgba(239,68,68,0.4)]" />
          <div className="h-2.5 w-2.5 rounded-full bg-yellow-500/90 shadow-[0_0_6px_rgba(234,179,8,0.35)]" />
          <div className="h-2.5 w-2.5 rounded-full bg-green-500/90 shadow-[0_0_6px_rgba(34,197,94,0.35)]" />
        </div>
      ...
      >
        <code className="grid gap-y-1.5 leading-relaxed">{wrappedChildren}</code>
      </pre>
    </div>
  )
